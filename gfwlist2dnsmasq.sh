...
get_args(){
    ...
    if [ $OUT_TYPE = 'SMARTDNS_RULES' ]; then
        ...
        GROUPNAME_TEST=$(echo $SMARTDNS_GROUP | grep -E '^\w+(,\w+)*$')
        if [ "$GROUPNAME_TEST" != "$SMARTDNS_GROUP" ]; then
            _red 'Error: Please enter a valid smartdns group name.\n'
            exit 1
        fi
    fi
...
process(){
    ...
    # Convert domains into dnsmasq rules
        ...
        if [ $OUT_TYPE = 'SMARTDNS_RULES' ]; then
            sort -u $DOMAIN_FILE | $SED_ERES 's#(.+)#''namespace /\1/'$SMARTDNS_GROUP'#g' > $CONF_TMP_FILE
            # Generate output file
